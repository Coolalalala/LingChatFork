...
export const useLlmProvidersStore = defineStore("llm-providers", {
  ...
  getters: {
    chatProvider: (state) => state.providers.find((p) => p.id === state.chatProviderId) ?? null,
    ...
    effectiveTranslateProvider: (state) => {
      if (state.translateProviderId) {
        return state.providers.find((p) => p.id === state.translateProviderId) ?? null;
      }
      return state.providers.find((p) => p.id === state.chatProviderId) ?? null;
    },
  ...
  actions: {
    ...
    async assignRole(
      role: "chat" | "translate" | "god_agent" | "vision" | "dreams" | "quick",
